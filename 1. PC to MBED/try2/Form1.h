...
/********INTERFACE**********************
 *
 * PC can send following instructions to MBED:
 *
 * d	(no params)		Read distance
 * c	(no params)		Read card code
 * r	(x tokens)		Dispane x number of red tokens
 * g	(x tokens)		Dispane x number of green tokens
 * b	(x tokens)		Dispane x number of blue tokens
 *
****************************************/

/********GET_REPLY***********************
 *
 * After instruction is sent to MBED, there should be reply
 * so this function is waiting for text from MBED and display
 * it in debug window.
 *
 * In case of error it is returning -1
 * In case of success it is returning reply in form of integer
 *
****************************************/
int get_reply(void) {
...
/********FORM - Load********************
 *
 * Get all COM porst and list it in comboBox
 *
****************************************/
private: System::Void Form1_Load(System::Object^  sender, System::EventArgs^  e) {

	// List all avaiable COM ports in comboBox
...
/********Init Button - Click************
 *
 * Set up connection with MBED thorugh COM port
 *
****************************************/
...
/********Go Button - Click**************
 *
 * Go button is asking for distance reader value
 * Command is d is for reading distance
 *
****************************************/
...
/********ReadCard Button - Click********
 *
 * Go button is asking for card reader value
 * Command is c is for reading distance
 *
****************************************/
 ...
 *
 * Go button is dispensing tokens
 *
 * Function is taking number of tokens and color from proper controls
 * And send instruction to MBED where it is analysed; for details go to MBED code
 *
 * For reply it is getting first number of tokens dispensed (after each token beeing dispensed)
 * and then final OK/Fail for whole procedure. It should also recieve OK/Fail for changing color
 *
****************************************/
private: System::Void dispenseGo_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			if(isError != 1){
				serialPort1->WriteLine(command);
				textBox1->AppendText("Reading reply" + Environment::NewLine);
				for (int i = 0; i <= Convert::ToInt16(coinsNo->Value); i++) {
					get_reply();
				}
			}
		 }
